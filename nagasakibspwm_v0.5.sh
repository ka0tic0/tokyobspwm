...
print_banner() {
 ...
 |_|\_/_/ \_\___/_/ \_\___/_/ \_\_|\_\___| |___/|____/|____/  \_/\_/  |_|  |_|
             -> Arch Linux Automated Installer & Catppuccin Mocha BSPWM
EOF
    echo -e "${CLR_RESET}"
...
setup_directories_and_backup() {
    ...
    if [ "$BACKUP_NEEDED" = true ]; then
        ...
        for conf in bspwm sxhkd polybar picom dunst rofi kitty fastfetch tmux nvim gtk-3.0 gtk-4.0; do
...
# Iniciar Nagasaki Banner en consolas interactivas
...
# Iniciar Nagasaki Banner en consolas interactivas (solo si es terminal interactiva)
...
configure_tmux() {
    log_info "Configurando Tmux (~/.config/tmux/tmux.conf)..."
    local TMUX_DIR="$CONFIG_DIR/tmux"
    run_as_user mkdir -p "$TMUX_DIR"
    local TMUX_CONF="$TMUX_DIR/tmux.conf"

    cat << 'EOF' > "$TMUX_CONF"
# ==============================================================================
# TMUX CONFIGURATION - NAGASAKI BSPWM (CATPPUCCIN MOCHA)
# ==============================================================================
...
setw -g window-status-format "#[fg=#45475a,bg=#1e1e2e] #I:#W "
setw -g window-status-current-format "#[fg=#11111b,bg=#cba6f7,bold] #I:#W #[fg=#cba6f7,bg=#1e1e2e] "

set -g status-right-length 100
set -g status-right "#[fg=#89b4fa,bg=#1e1e2e]󰣇 nagasaki #[fg=#a6e3a1,bg=#1e1e2e] 󰍛 %H:%M "

set -g pane-border-style "fg=#313244"
set -g pane-active-border-style "fg=#cba6f7"
EOF
    chown -R "$TARGET_USER:$TARGET_USER" "$TMUX_DIR"
    log_success "Tmux configurado con interfaz Catppuccin Mocha."
}
...
modules-right = date pulseaudio memory sysmenu
...
modules-left = launcher bspwm xwindow
modules-center = date
modules-right = memory cpu pulseaudio sysmenu
...
modules-left = launcher bspwm xwindow
modules-center = date
modules-right = filesystem memory cpu pulseaudio battery sysmenu
...
module-margin = 1
font-0 = "JetBrainsMono Nerd Font:size=10:weight=bold;3"
modules-right = cpu memory pulseaudio battery sysmenu
enable-ipc = true
wm-restack = bspwm
...
modules-left = bspwm xwindow
modules-center = 
modules-right = pulseaudio memory cpu battery network date sysmenu
...
EOF
    ...
    chmod +x "$LAUNCH_POLY" "$WALL_SCRIPT" "$THEME_SELECT" "$POLYTHEME_CLI" "$POWERMENU" "$MASTER_MENU" "$NOTIFY_VOL" "$NOTIFY_BRI" "$WIFI_MENU" "$BT_MENU" "$AUDIO_MENU" "$CLIP_MENU" "$MON_MENU"
    chown -R "$TARGET_USER:$TARGET_USER" "$BIN_DIR"
    log_success "Scripts de utilidades, applets de Rofi y OSDs creados con permisos de ejecución."
}
