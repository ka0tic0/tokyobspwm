...
install_packages() {
  msg_info "Instalando paquetes principales..."
...
install_optional() {
  msg_info "Instalación de paquetes opcionales"
  read -p "¿Deseas instalar paquetes opcionales (brave-bin, neovim, htop, fastfetch, tokyonight-gtk-theme-git, tela-circle-icon-theme-nord)? [S/n]: " opt_install
  if [[ "$opt_install" =~ ^[Ss]$ ]] || [[ -z "$opt_install" ]]; then
    yay -S --needed --noconfirm brave-bin neovim htop fastfetch tokyonight-gtk-theme-git tela-circle-icon-theme-nord
    msg_ok "Paquetes opcionales instalados."
  else
...
configure_polybar() {
  ...
  git clone https://github.com/khanhas/Polybar.git /tmp/Polybar-khanhas || true
...
if [ -n "$chosen" ]; then
    ...
    if [ -f "$HOME/.config/polybar/themes/$theme_lower/config.ini" ]; then
        ...
        ~/.local/bin/launch_polybar
        dunstify -i dialog-information "Polybar" "Tema activo: $chosen"
    fi
fi
EOF
...
if [ ! -d "$WALL_DIR" ]; then
  dunstify -i dialog-warning "Wallpapers" "No existe la carpeta $WALL_DIR"
  exit 1
fi
...
if [ -n "$selected" ]; then
    ...
    if [ -n "$FULL_PATH" ]; then
        ...
        dunstify -i dialog-information "Wallpaper" "Fondo de pantalla actualizado"
    fi
fi
EOF
...
case "$chosen" in
    ...
    "$opt_wall_rand")
        ~/.local/bin/set_random_wallpaper
        dunstify -i dialog-information "Wallpaper" "Nuevo fondo aleatorio aplicado"
        ;;
    "$opt_lock")
        i3lock -c 1a1b26
        ;;
...
download_wallpaper() {
  msg_info "Descargando Wallpapers..."
  mkdir -p ~/Pictures/wallpapers
  if [ ! -d "/tmp/walls" ]; then
    git clone https://github.com/dharmx/walls.git /tmp/walls || true
  fi
  if [ -d "/tmp/walls" ]; then
...
post_install() {
  ...
  systemctl --user enable pipewire wireplumber --now || true
