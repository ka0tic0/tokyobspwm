...
notify "Preparando escenario estético para captura de GitHub..."

kitty --title "Nagasaki Fetch" sh -c "~/.local/bin/nagasaki_fetch 2>/dev/null || fastfetch; sleep 10" &
sleep 0.5

if command -v cava &>/dev/null; then
    kitty --title "Nagasaki Visualizer" sh -c "cava; sleep 10" &
    sleep 0.5
fi
...
if [ -f "$FILE" ]; then
    xclip -selection clipboard -t image/png -i "$FILE" 2>/dev/null || true
    notify "Captura de Showcase guardada en:\n$FILE\n(Copiada al portapapeles)"
fi
EOF
