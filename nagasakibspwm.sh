...
install_official_packages() {
    ...
    local PACKAGES=(
        ...
        # Audio (Pipewire stack & Pulse utilities)
        pipewire
        pipewire-pulse
        pipewire-alsa
        wireplumber
        ...
        # Display Manager & Control del Sistema
        sddm
        brightnessctl
        playerctl
...
# ------------------------------------------------------------------------------
setup_rices_engine() {
    log_info "5.1 Configurando Motor Multi-Rice Global (Tokyo, Osaka, Kyoto, Yokohama, Nikko, Catppuccin, Dracula, Gruvbox)..."

    local RICES_DIR="$CONFIG_DIR/bspwm/rices"
    run_as_user mkdir -p "$RICES_DIR"

    local THEMES=(
        "Tokyo|#7aa2f7|#bb9af7|#1a1b26|#c0caf5|Tokyo Night Pills (gh0stzk Pamela style)|98%|1%|8pt|10"
        "Osaka|#fab387|#cba6f7|#1e1e2e|#cdd6f4|Osaka Sunset Floating Dock (gh0stzk Brenda style)|96%|2%|10pt|14"
        "Kyoto|#7fbbb3|#a7c080|#2b3339|#d3c6aa|Kyoto Emerald Mac Island (gh0stzk Melissa style)|98%|1%|6pt|8"
        "Yokohama|#80deea|#ff80ab|#212121|#eeffff|Yokohama Material Blocks|100%|0%|0pt|0"
        "Nikko|#88c0d0|#81a1c1|#2e3440|#d8dee9|Nikko Nordish Badges|98%|1%|6pt|10"
        "CatppuccinMocha|#89b4fa|#cba6f7|#1e1e2e|#cdd6f4|Catppuccin Mocha Default|100%|0%|0pt|0"
        "Dracula|#bd93f9|#ff79c6|#282a36|#f8f8f2|Dracula Dark Violet|98%|1%|8pt|12"
        "Gruvbox|#d79921|#fe8019|#282828|#ebdbb2|Gruvbox Retro Gold|100%|0%|0pt|0"
    )

    for theme_info in "${THEMES[@]}"; do
        IFS='|' read -r t_name t_border t_accent t_bg t_fg t_desc t_w t_offx t_offy t_rad <<< "$theme_info"
...
EOF
        ...
        cat << EOF > "$t_dir/polybar.ini"
; ==============================================================================
; POLYBAR CONFIGURATION - RICE: $t_name ($t_desc)
; ==============================================================================
...
EOF
    ...
    log_success "Motor Multi-Rice global de 8 temas con Polybar independiente configurado en $RICES_DIR."
}
...
case "$CHOSEN" in
    ...
    *9.*) ~/.local/bin/launch_polybar; notify-send "Polybar" "Polybar recargada exitosamente" ;;
esac
EOF
