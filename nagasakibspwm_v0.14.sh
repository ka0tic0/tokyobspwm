...
# Nagasaki Quick Control Dashboard (Eww / Rofi)
super + shift + a
    ~/.local/bin/nagasaki_dashboard

# Gestión Inteligente de Ventanas & Layouts
super + shift + w
    ~/.local/bin/bspwm_layout_picker
...
modules-left = bspwm layout-picker-icon xwindow
modules-center = date
modules-right = spotify dashboard-icon cpu memory filesystem pulseaudio network battery sysmenu
...
[module/layout-picker-icon]
type = custom/text
content = " 🪟 "
content-foreground = ${colors.pink}
click-left = ~/.local/bin/bspwm_layout_picker

[module/dashboard-icon]
type = custom/text
content = " 🎛️ "
content-foreground = ${colors.mauve}
click-left = ~/.local/bin/nagasaki_dashboard
...
configure_eww() {
    log_info "Configurando Nagasaki Quick Control Dashboard en Eww (~/.config/eww)..."
    local EWW_DIR="$CONFIG_DIR/eww"
    run_as_user mkdir -p "$EWW_DIR"

    cat << 'EOF' > "$EWW_DIR/eww.yuck"
;; ==============================================================================
;; NAGASAKI QUICK CONTROL DASHBOARD (EWW YUCK CONFIG)
;; ==============================================================================

(defpoll volume :interval "1s" "pamixer --get-volume 2>/dev/null || echo 50")
(defpoll brightness :interval "1s" "brightnessctl -m 2>/dev/null | awk -F, '{print substr($4, 0, length($4)-1)}' || echo 50")
(defpoll memory :interval "2s" "free -m | awk '/Mem:/ { printf(\"%d\", $3/$2*100) }'")
...
(defpoll music_artist :interval "1s" "playerctl metadata artist 2>/dev/null || echo 'Nagasaki Audio'")
...
(defwidget header_section []
  (box :class "card" :orientation "horizontal" :space-evenly false :spacing 10
    (label :class "title" :text "🌿 Nagasaki Control Center")))

(defwidget metrics_section []
  (box :class "card" :orientation "horizontal" :space-evenly true
    (box :orientation "vertical" :space-evenly false :spacing 4
      (circular-progress :value cpu :thickness 6 :class "cpu-circle"
        (label :text "${round(cpu, 0)}%"))
      (label :class "subtext" :text "CPU"))
    (box :orientation "vertical" :space-evenly false :spacing 4
      (circular-progress :value memory :thickness 6 :class "ram-circle"
        (label :text "${memory}%"))
      (label :class "subtext" :text "RAM"))))

(defwidget controls_section []
  (box :class "card" :orientation "vertical" :space-evenly false :spacing 8
    (box :orientation "horizontal" :space-evenly false :spacing 10
      (label :text "󰕾")
      (scale :value volume :min 0 :max 100 :onchange "pamixer --set-volume {}"))
    (box :orientation "horizontal" :space-evenly false :spacing 10
      (label :text "󰃠")
      (scale :value brightness :min 5 :max 100 :onchange "brightnessctl set {}%"))))

(defwidget media_section []
  (box :class "card media-card" :orientation "vertical" :space-evenly false :spacing 6
    (label :class "song-title" :limit-width 22 :text music_title)
    (label :class "subtext" :limit-width 22 :text music_artist)
    (box :orientation "horizontal" :halign "center" :spacing 15
      (button :onclick "playerctl previous" "󰒮")
      (button :onclick "playerctl play-pause" "󰐊")
      (button :onclick "playerctl next" "󰒝"))))

(defwidget quick_tools_section []
  (box :class "card" :orientation "vertical" :space-evenly false :spacing 8
    (box :orientation "horizontal" :space-evenly true
      (button :class "action-btn" :onclick "~/.local/bin/rice_editor &" "✏️ Rice Editor")
      (button :class "action-btn" :onclick "~/.local/bin/bspwm_tweaks &" "🛠️ Tweaks"))
    (box :orientation "horizontal" :space-evenly true
      (button :class "action-btn" :onclick "~/.local/bin/bspwm_layout_picker &" "🪟 Layouts")
      (button :class "action-btn" :onclick "~/.local/bin/nagasaki_showcase &" "📸 Showcase"))))
EOF

    cat << 'EOF' > "$EWW_DIR/eww.scss"
// ==============================================================================
// NAGASAKI DASHBOARD SCSS STYLING (CATPPUCCIN MOCHA PALETTE)
// ==============================================================================
...
EOF
    chown -R "$TARGET_USER:$TARGET_USER" "$EWW_DIR"
    log_success "Eww Dashboard configurado con tema Catppuccin Mocha."
}
...
modules-left = launcher bspwm layout-picker-icon xwindow
modules-center = date
modules-right = spotify dashboard-icon filesystem pulseaudio memory cpu battery
...
modules-left = launcher bspwm layout-picker-icon
modules-center = xwindow
modules-right = spotify dashboard-icon date pulseaudio memory sysmenu
...
modules-left = launcher bspwm layout-picker-icon xwindow
modules-center = date
modules-right = spotify dashboard-icon filesystem memory cpu pulseaudio
...
modules-left = launcher bspwm layout-picker-icon xwindow
modules-center = date
modules-right = spotify dashboard-icon memory cpu pulseaudio sysmenu
...
modules-left = launcher bspwm layout-picker-icon
modules-center = date
modules-right = spotify dashboard-icon memory cpu pulseaudio
...
modules-left = launcher bspwm layout-picker-icon xwindow
modules-center = date
modules-right = spotify dashboard-icon filesystem memory cpu pulseaudio battery sysmenu
...
EOF
    ...
    # 7. DRACULA (Dark Violet & Rocket Style)
...
modules-left = launcher bspwm layout-picker-icon xwindow
modules-center = date
modules-right = spotify dashboard-icon cpu memory volume battery sysmenu
...
# Script: bspwm_layout_picker (Centro Inteligente de Gestión de Ventanas)
    local LAYOUT_PICKER="$BIN_DIR/bspwm_layout_picker"
    cat << 'EOF' > "$LAYOUT_PICKER"
#!/usr/bin/env bash
export PATH="$HOME/.local/bin:/usr/local/bin:/usr/bin:/bin:$PATH"
export DISPLAY="${DISPLAY:-:0}"

notify() {
    notify-send -u low -i window-new "Nagasaki Window Manager" "$1" 2>/dev/null || true
}

MENU_OPTIONS="1. 📐 Modo Tiled (Mosaico)\n2. 🔍 Modo Monocle (Pantalla Única)\n3. 🪟 Modo Flotante (Floating)\n4. ⚖️ Igualar Tamaño de Ventanas (Equalize)\n5. 🌳 Balancear Árbol de Ventanas (Balance)\n6. 🔄 Rotar Disposición 90° (Rotate)\n7. 👑 Promover a Ventana Principal (Master)\n8. 📌 Alternar Fijado en Pantallas (Sticky)\n9. 🔒 Alternar Bloqueo de Ventana (Lock)\n10. 🔍 Buscar y Enfocar Ventana Abierta"

CHOSEN=$(echo -e "$MENU_OPTIONS" | rofi -dmenu -i -p "Gestión de Ventanas:" -theme ~/.config/rofi/config.rasi 2>/dev/null || echo -e "$MENU_OPTIONS" | rofi -dmenu -i -p "Gestión de Ventanas:")

case "$CHOSEN" in
    "1."*)
        bspc node -t tiled
        notify "Layout cambiado a: Tiled (Mosaico)"
        ;;
    "2."*)
        bspc node -t monocle
        notify "Layout cambiado a: Monocle (Pantalla Única)"
        ;;
    "3."*)
        bspc node -t ~floating
        notify "Estado de ventana cambiado a: Flotante"
        ;;
    "4."*)
        bspc node @/ -e 2>/dev/null || bspc desktop -E 2>/dev/null || true
        notify "Proporciones de ventanas igualadas (Equalize)."
        ;;
    "5."*)
        bspc desktop -B 2>/dev/null || true
        notify "Árbol de ventanas balanceado (Balance)."
        ;;
    "6."*)
        bspc node @/ -R 90 2>/dev/null || true
        notify "Disposición rotada 90°."
        ;;
    "7."*)
        bspc node -s biggest 2>/dev/null || true
        notify "Ventana promovida a Master (Principal)."
        ;;
    "8."*)
        bspc node -g sticky
        notify "Estado Sticky (Fijado en pantallas) alternado."
        ;;
    "9."*)
        bspc node -g locked
        notify "Estado de Bloqueo alternado."
        ;;
    "10."*)
        rofi -show window -theme ~/.config/rofi/config.rasi 2>/dev/null || rofi -show window
        ;;
esac
# Script: nagasaki_dashboard (Lanzador Eww / Rofi Dashboard)
    local DASHBOARD_SCRIPT="$BIN_DIR/nagasaki_dashboard"
...
if command -v eww &>/dev/null; then
    ...
    OPTIONS="🔊 Ajustar Volumen (+5% / -5%)\n☀️ Ajustar Brillo (+5% / -5%)\n🌙 Modo Noche (Filtro Cálido 4500K)\n🎮 Modo Gaming (Picom ON/OFF)\n🔕 Modo No Molestar (Dunst DND)\n🧠 Liberar Memoria RAM (Drop Caches)\n✏️ Rice Editor (Ajustar Tema)\n🛠️ Mantenimiento & Tweaks\n🪟 Gestión Inteligente de Ventanas\n📸 Nagasaki Showcase\n⚡ Menú de Apagado"

    CHOSEN=$(echo -e "$OPTIONS" | rofi -dmenu -i -p "Nagasaki Control Dashboard:" -theme ~/.config/rofi/config.rasi 2>/dev/null || echo -e "$OPTIONS" | rofi -dmenu -i -p "Nagasaki Control Dashboard:")

    case "$CHOSEN" in
        *Volumen*) ~/.local/bin/notify_volume up ;;
        *Brillo*) ~/.local/bin/notify_brightness up ;;
        *Noche*) ~/.local/bin/night_mode ;;
        *Gaming*) 
            if pgrep -x picom >/dev/null; then
                pkill picom
                notify-send -u low -i preferences-system "Modo Gaming" "Picom deshabilitado" 2>/dev/null || true
            else
                picom --config ~/.config/picom/picom.conf -b &
                notify-send -u low -i preferences-system "Modo Gaming" "Picom habilitado" 2>/dev/null || true
            fi
            ;;
        *No*Molestar*) dunstctl set-paused toggle 2>/dev/null || true ;;
        *RAM*) sudo sync && echo 3 | sudo tee /proc/sys/vm/drop_caches >/dev/null 2>&1 || true ;;
        *Editor*) ~/.local/bin/rice_editor ;;
        *Tweaks*|*Mantenimiento*) ~/.local/bin/bspwm_tweaks ;;
        *Ventanas*|*Layout*) ~/.local/bin/bspwm_layout_picker ;;
        *Showcase*) ~/.local/bin/nagasaki_showcase ;;
        *Apagado*) ~/.local/bin/powermenu_rofi ;;
    esac
fi
EOF

    chmod +x "$LAUNCH_POLY" "$WALL_SCRIPT" "$THEME_SELECT" "$POLYTHEME_CLI" "$POWERMENU" "$MASTER_MENU" "$NOTIFY_VOL" "$NOTIFY_BRI" "$WIFI_MENU" "$BT_MENU" "$AUDIO_MENU" "$CLIP_MENU" "$MON_MENU" "$NAGASAKI_FETCH" "$SPOTIFY_STATUS" "$LAUNCH_MUSIC" "$RICE_EDITOR" "$NOTIFY_MEDIA" "$ROFI_SYS" "$NIGHT_SCRIPT" "$RECENT_FILES" "$BLUR_LOCK_SCRIPT" "$PWR_PROF" "$TWEAKS_SCRIPT" "$WELCOME_SCRIPT" "$SHOWCASE_SCRIPT" "$LAYOUT_PICKER" "$DASHBOARD_SCRIPT" 2>/dev/null || true
