...
setup_rices_engine() {
    ...
    local THEMES=(
        ...
        "Matsuyama|#d79921|#fe8019|#cc282828|#ebdbb2|Matsuyama Autumn Gold & Crimson Glass"
    )
...
EOF
    ...
    # 9. HIROSHIMA (Cyber-Sunset Neon Glass Dock)
...
modules-left = launcher bspwm sep-pill xwindow
modules-center = date
...
EOF
    ...
    # 11. MATSUYAMA (Autumn Gold & Crimson Glass)
...
modules-left = launcher bspwm xwindow
modules-center = date
modules-right = memory cpu pulseaudio battery sysmenu
...
type = internal/xwindow
label = %title:0:25:...%
label-foreground = ${colors.crimson}
...
type = internal/pulseaudio
label-volume = "󰕾 %percentage%%"
label-volume-foreground = ${colors.crimson}
...
EOF
    ...
    chmod +x "$LAUNCH_POLY" "$WALL_SCRIPT" "$THEME_SELECT" "$POLYTHEME_CLI" "$POWERMENU" "$MASTER_MENU" "$NOTIFY_VOL"
    chown -R "$TARGET_USER:$TARGET_USER" "$BIN_DIR"
