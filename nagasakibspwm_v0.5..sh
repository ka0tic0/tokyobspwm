...
setup_directories_and_backup() {
    ...
    if [ "$BACKUP_NEEDED" = true ]; then
        ...
        for conf in bspwm sxhkd polybar picom dunst rofi kitty fastfetch nvim gtk-3.0 gtk-4.0; do
...
EOF
    ...
    chmod +x "$LAUNCH_POLY" "$WALL_SCRIPT" "$THEME_SELECT" "$POLYTHEME_CLI" "$POWERMENU" "$MASTER_MENU" "$NOTIFY_VOL" "$NOTIFY_BRI" "$WIFI_MENU" "$BT_MENU" "$AUDIO_MENU" "$CLIP_MENU" "$MON_MENU" "$NAGASAKI_FETCH" 2>/dev/null || true
    chmod +x "$BIN_DIR"/* 2>/dev/null || true
    chown -R "$TARGET_USER:$TARGET_USER" "$BIN_DIR"
    log_success "Scripts de utilidades, applets de Rofi, OSDs y Nagasaki Fetch creados con permisos de ejecución."
}
